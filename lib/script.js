  ...
  mounted() {
    ...
    const getNominalRate = () => {
      if (this.options.autoScrollRate === 10 && videoState.playing) {
        const target = autoScroller.getScrollingPositionAtTime(videoState.currentTime)
        if (target !== null) return (target - window.scrollY) * 0.25
      }
      return autoScroller.getScrollingRateAtScrollingPosition(Math.floor(window.scrollY))
    }
    const frame = () => {
...
Vue.component('chord-block', {
  ...
  template: `<button class="chord-block" :class="{'is-active':active,'is-empty':empty}" :data-interval="halfStepsFromTonic" @click="seek">
    {{text}}
  </button>`,
  mounted() {
    this.cb = () => {
