...
function startPlayingChord(name) {
  ...
  if (chord.chord[chord.chord.length - 1].octave > 5) chord = chord.inOctave(chord.octave - 1)
...
if (window.location.search.match(/postToParent/) && window.parent) {
  ...
  if (isDevMode) {
    window.addEventListener('keydown', (e) => {
      if (e.keyCode === 'R'.charCodeAt(0) && !e.ctrlKey && !e.altKey && !e.metaKey) {
        e.preventDefault()
        const iframe = document.createElement('iframe')
        iframe.setAttribute('style', 'position: fixed; top: 0; left: 0')
        iframe.src = location.href.replace(/\?[^]*|$/, '?postToParent=' + Date.now())
